...
public class BackSlotHotbarRendererProcedure {
	...
	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			BackSlotHotbarRendererProcedure.targetStage = targetStage;
			return true;
		} else if (targetStage == 1) {
			if (currentStage != 0) {
				BackSlotHotbarRendererProcedure.targetStage = currentStage;
				return true;
			}
		}
		return false;
	...
	private static void execute(@Nullable Event event) {
		...
		double slotY = 0;
		double slotX = 0;
		boolean rendered_already = false;
		midX = Math.floor(Minecraft.getInstance().getWindow().getGuiScaledWidth() / 2d);
		midY = Minecraft.getInstance().getWindow().getGuiScaledHeight();
		if (Minecraft.getInstance().player != null) {
			...
			if (target(1)) {
				if (Minecraft.getInstance().screen == null || Minecraft.getInstance().screen instanceof InventoryScreen) {
					if (!(entity.getData(PokerelicsModVariables.PLAYER_VARIABLES).backSlot.getItem() == Blocks.AIR.asItem())) {
						slotX = midX + 98;
						slotY = midY - 22;
						if (rendered_already != true) {
							RenderSystem.setShaderTexture(0, ResourceLocation.parse(("pokerelics" + ":textures/" + "offhand_icon" + ".png")));
							renderTexture((float) slotX, (float) slotY, 220, 0, 1, 255 << 24 | 255 << 16 | 255 << 8 | 255, 0);
