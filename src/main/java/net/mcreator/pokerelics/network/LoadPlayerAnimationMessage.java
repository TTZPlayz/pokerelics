...
public record LoadPlayerAnimationMessage(String animationfile) implements CustomPacketPayload {
	public static final Type<LoadPlayerAnimationMessage> TYPE = new Type<>(ResourceLocation.fromNamespaceAndPath(PokerelicsMod.MODID, "load_player_animation"));
	...
	public static void handleData(final LoadPlayerAnimationMessage message, final IPayloadContext context) {
		if (context.flow() == PacketFlow.CLIENTBOUND) {
			context.enqueueWork(() -> {
				...
				PokerelicsModPlayerAnimationAPI.loadAnimationFile(received);
			}).exceptionally(e -> {
	...
	public static void registerMessage(FMLCommonSetupEvent event) {
		PokerelicsMod.addNetworkMessage(LoadPlayerAnimationMessage.TYPE, LoadPlayerAnimationMessage.STREAM_CODEC, LoadPlayerAnimationMessage::handleData);
	}
}